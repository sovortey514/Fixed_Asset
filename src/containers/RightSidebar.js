...
function RightSidebar(){
    ...
    const {isOpen, bodyType,  header} = useSelector(state => state.rightDrawer)
    ...
    return(
        <div className={" fixed overflow-hidden z-20 bg-opacity-25 inset-0 transform ease-in-out " + (isOpen ? " transition-opacity opacity-100 duration-500 translate-x-0  " : " transition-all delay-500 opacity-0 translate-x-full  ")}>
      
            <section className={ "w-80 md:w-96  right-0 absolute h-full shadow-xl delay-400 duration-500 ease-in-out transition-all transform  " + (isOpen ? " translate-x-0 " : " translate-x-full ")}>
